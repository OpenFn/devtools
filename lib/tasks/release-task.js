...
module.exports = async function releaseTask({
  ...
  async function publish({ adaptorPath, dryRun }) {
    const npmPrefix = await execa.stdout('npm', ['config', 'get', 'prefix']);
    console.log('npmPrefix', npmPrefix);
    await execa(
      'sh',
      ['-c', `npm publish --prefix ${npmPrefix} --access=public${dryRun ? ' --dry-run' : ''}`],
      {
        cwd: adaptorPath,
        env: process.env
      }
  ...
  const versionTask = {
    ...
    task: async ({ adaptorPath, bumpType }) => {
      if (!bumpType) {
        throw new Error('Version bumpType required for versionTask.');
      }

      return execa('sh', ['-c', `npm version ${bumpType}`], {
        cwd: adaptorPath,
      });
    },
  };
  ...
  if (bumpVersion) {
    ...
    tasks.unshift(versionTask);
  }

  return publish({ adaptorPath, dryRun });
};
