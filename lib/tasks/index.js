...
function ContextualError(message, ctx) {
  this.ctx = ctx;
...
exports.packageRelease = function packageRelease({
  adaptorPath,
  uploadToGithub,
}) {
  ...
  const packTask = {
    title: 'npm pack all',
    task: () => {
      return execa('npx', ['npm-pack-all'], { cwd: adaptorPath });
    },
  };
  ...
  const uploadTask = {
    ...
    task: () =>
      uploadRelease({
        ...
        tag: `v${version}`,
        tarballPath: path.join(adaptorPath, `${name}-${version}.tgz`),
      }).run(),
    enabled: () => uploadToGithub,
  };

  return new Listr([packTask, uploadTask]);
};
